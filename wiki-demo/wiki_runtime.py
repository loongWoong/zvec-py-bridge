...
def create_document(
    ...
    """, {
        ...
        "code_symbols": code_symbols, "commit_hash": commit_hash, "commit_ts": commit_ts,
    })
...
# ====================================================================== #
#  W3.1 检索端消费 chunk 级概念
#  在 query 时对每个命中 chunk 确定性地恢复其所属概念（approved），
#  使用与 pipeline._annotate_chunk_concepts 完全相同的逻辑，因此无需依赖
#  zvec 落库 concept_ids 字段即可获得「chunk 级」的精确概念集合——比文档级
#  concept_ids（整篇文档概念的超集）更精细，使 Re-Rank 的概念距离因子真正按
#  「命中的那段内容」生效，而非被整篇文档的无关概念稀释。
# ====================================================================== #
_CONCEPT_MAP_CACHE: dict = {"ts": 0.0, "map": {}}


def _chunk_concept_ids(text: str, heading: str = "") -> list[str]:
    """根据 chunk 文本确定性匹配其所属概念（approved），返回 concept_id 列表。

    结果缓存 60s（概念表不常变），避免每次向量检索都打 DB。
    """
    now = time.time()
    if now - _CONCEPT_MAP_CACHE["ts"] > 60 or not _CONCEPT_MAP_CACHE["map"]:
        try:
            import ontology as _onto
            concepts = _onto.list_concepts(status="approved")
        except Exception:
            return []
        _CONCEPT_MAP_CACHE["map"] = {
            c["name"].lower(): c["id"]
            for c in concepts if c.get("name")
        }
        _CONCEPT_MAP_CACHE["ts"] = now
    name_to_id = _CONCEPT_MAP_CACHE["map"]
    if not name_to_id:
        return []
    blob = f"{heading} {text}".lower()
    return [cid for nm, cid in name_to_id.items() if len(nm) >= 2 and nm in blob]
...
# W4.1：commit 时间戳缓存（git 查询较重，按 commit hash 缓存）
_COMMIT_TS_CACHE: dict[str, int | None] = {}


def _git_commit_ts(commit_hash: str, repo_dir: str | None = None) -> int | None:
    """解析某 commit 的 author 时间戳（epoch 秒），用于 since_commit 区间过滤。

    优先读缓存；git 不可达时返回 None（调用方退化为精确匹配）。
    """
    if not commit_hash:
        return None
    if commit_hash in _COMMIT_TS_CACHE:
        return _COMMIT_TS_CACHE[commit_hash]
    ts: int | None = None
    try:
        import subprocess
        ...
        s = out.stdout.strip()
        ts = int(s) if s.isdigit() else None
    except Exception:
        ts = None
    _COMMIT_TS_CACHE[commit_hash] = ts
    return ts


def _filter_since_commit(results: list[dict], since_commit: str | None) -> list[dict]:
    """W4.1：按 commit 区间过滤检索结果。

    语义：仅保留「提交时间 >= since_commit 提交时间」的文档，即"该提交之后
    变更过的文档"。依赖 document.commit_ts（epoch 秒）字段；若旧文档未存
    commit_ts，则回退到其 commit_hash 的时间戳。git 完全不可用时退化为
    精确匹配该 commit_hash（兼容旧行为）。
    """
    ...
    try:
        rows = _q(
            "SELECT id, commit_hash, commit_ts FROM document WHERE id IN $ids",
            {"ids": ids},
        )
        cmap: dict[str, tuple] = {}
        for d in _flatten(rows):
            if isinstance(d, dict):
                cmap[_extract_id(d.get("id", ""))] = (
                    d.get("commit_hash"), d.get("commit_ts"),
                )
    except Exception:
        return results

    cutoff = _git_commit_ts(since_commit)
    if cutoff is None:
        # git 不可用：退化为精确匹配（原行为）
        return [r for r in results
                if cmap.get(r.get("doc_id", ""), (None, None))[0] == since_commit]

    kept = []
    for r in results:
        did = r.get("doc_id", "")
        ch, cts = cmap.get(did, (None, None))
        if cts is None and ch:
            cts = _git_commit_ts(ch)  # 旧文档回退到其 commit 的时间戳
        if cts is not None and cts >= cutoff:
            kept.append(r)
        elif ch == since_commit:
            kept.append(r)  # 边界：恰好命中该 commit
    return kept
...
def search_documents(query: str, topk: int = 5,
        ...
        try:
            ...
            for rank, c in enumerate(all_candidates):
                ...
                if did not in rrf_ranked:
                    rrf_ranked[did] = {"score": 0.0, "sources": set(),
                                       "title": c.get("title", ""),
                                       "excerpt": c.get("excerpt", ""),
                                       "chunk_concept_ids": set()}
                rrf_ranked[did]["score"] += rrf
                if route:
                    rrf_ranked[did]["sources"].add(route)
                # W3.1：聚合各路候选带回的 chunk 级概念
                rcids = c.get("concept_ids") or []
                if rcids:
                    rrf_ranked[did]["chunk_concept_ids"].update(rcids)

            # 附加文档级概念，使 Re-Rank 概念距离因子生效
            _attach_concept_ids(rrf_ranked)

            # W3.1：用 chunk 级精确概念覆盖文档级（命中段落更精细）；
            # 若某候选无 chunk 级概念（如仅命中 fts/graph 路）则保留文档级。
            for _did, _info in rrf_ranked.items():
                _ccids = _info.pop("chunk_concept_ids", set())
                _info["matched_concept_ids"] = sorted(_ccids) if _ccids else []
                if _ccids:
                    _info["concept_ids"] = sorted(_ccids)
            ...
            # W3.1：把 chunk 级命中概念透出到结果，便于前端/调用方观察
            _matched_map = {_d: _i.get("matched_concept_ids", [])
                            for _d, _i in rrf_ranked.items()}
            for _rr in reranked:
                _rr["matched_concept_ids"] = _matched_map.get(_rr.get("doc_id", ""), [])
    ...
    for rank, c in enumerate(all_candidates):
        ...
        if did not in ranked:
            ranked[did] = {"score": 0.0, "sources": set(),
                           "title": c.get("title", ""), "excerpt": c.get("excerpt", ""),
                           "chunk_concept_ids": set()}
        ...
        rcids = c.get("concept_ids") or []
        if rcids:
            ranked[did]["chunk_concept_ids"].update(rcids)

    # 附加文档级概念
    _attach_concept_ids(ranked)

    # W3.1：chunk 级概念覆盖文档级（命中段落更精细）
    for _did, _info in ranked.items():
        _ccids = _info.pop("chunk_concept_ids", set())
        _info["matched_concept_ids"] = sorted(_ccids) if _ccids else []
        if _ccids:
            _info["concept_ids"] = sorted(_ccids)
    ...
    for doc_id, info in sorted(ranked.items(), key=lambda x: -x[1]["score"]):
        results.append({
            ...
            "sources": sorted(info["sources"]),
            "concept_ids": info.get("concept_ids", []),
            "matched_concept_ids": info.get("matched_concept_ids", []),
            "excerpt": info["excerpt"],
        })
...
def _search_vector(query: str, topk: int) -> list[dict]:
    ...
    W3.1：聚合每个命中 chunk 的「chunk 级概念」（优先用 zvec 返回的
    concept_ids，缺失时确定性重算），按 doc_id 求并集，使后续 Re-Rank
    能用命中段落的精确概念而非整篇文档的概念超集。
    """
    try:
    ...
    for ch in chunks:
        ...
        cids = ch.get("concept_ids") or _chunk_concept_ids(
            ch.get("content", ""), ch.get("heading", "")
        )
        entry = agg.get(did)
        if entry is None:
            entry = {
                ...
                "concept_ids": set(cids),
            }
            agg[did] = entry
        else:
            if score > entry["vector_similarity"]:
                entry["vector_similarity"] = score
                entry["excerpt"] = ch.get("content", "")[:120]
                entry["title"] = ch.get("title", "")
            entry["concept_ids"].update(cids)
    results = []
    for e in agg.values():
        e["concept_ids"] = sorted(e["concept_ids"])
        results.append(e)
    return results
